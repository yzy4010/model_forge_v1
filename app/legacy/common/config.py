# ==============================================================
# 【本地运行说明】
# - Redis: 默认 127.0.0.1:6379/db3，无密码（如有需要可修改下方 REDIS_PASSWORD）
# - Java:  默认 http://localhost:8081（RuoYi 后端端口）
# - 文件路径: 原为 Linux `/data/...` 路径，本地运行如目录不存在会自动创建
# - 设备: 默认 CPU（如有 CUDA 可取消注释 cuda 行，注释 cpu 行）
# - SSH 服务器配置用于推理服务器间文件同步，本地不需要
# ==============================================================

# 部署类型
# ⚠️ 注意：此常量仅作为旧版模块的内部记录，实际控制启动行为的是 app/config.py 中的 PUBLIC_TYPE（从环境变量读取）。
#    修改此处不会影响服务类型。请通过环境变量 PUBLIC_TYPE=1（推理）/ PUBLIC_TYPE=2（训练）控制。
