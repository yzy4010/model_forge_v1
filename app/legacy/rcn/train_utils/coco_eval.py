...
class EvalCOCOMetric:
    ...
    def synchronize_results(self):
        ...
        if is_main_process():
            ...
            with open(self.results_file_name, 'w',encoding="utf-8") as json_file:
    ...
    def evaluate(self):
        ...
        if is_main_process():
            ...
            # 删除 small、medium、large
            print(f"len(coco_info): {len(coco_info)}")
            if len(coco_info)==10:
                coco_info.pop(9)
                coco_info.pop(8)
                coco_info.pop(4)
                coco_info.pop(3)
            elif len(coco_info)==12:
                coco_info.pop(11)
                coco_info.pop(10)
                coco_info.pop(9)
                coco_info.pop(5)
                coco_info.pop(4)
                coco_info.pop(3)
            print(f"len(coco_info)1: {len(coco_info)}")
