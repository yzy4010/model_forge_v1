...
def summarize(self, catId=None):
    ...
    def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100):
        ...
        """
        :param ap: 1代表计算precision，0代表计算recall
        :param iouThr: iou阈值
        :param areaRng: 区域范围
        :param maxDets: 最大检测数量
        :return:
        """
    stats, print_list = [0] * 6, [""] * 6
    stats[0], print_list[0] = _summarize(1)
    stats[1], print_list[1] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[2])
    stats[2], print_list[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
    stats[3], print_list[3] = _summarize(0, maxDets=self.params.maxDets[0])
    stats[4], print_list[4] = _summarize(0, maxDets=self.params.maxDets[1])
    stats[5], print_list[5] = _summarize(0, maxDets=self.params.maxDets[2])
    '''
