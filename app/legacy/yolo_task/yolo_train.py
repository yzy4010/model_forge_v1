...
# 训练方法
def action_first(request: TrainParameter, csv_name: str, trainingRecord_id: str, project_name: str, action_name: str,
    ...
    # if osName == "windows":
    #     run_script(f"yolo detect train data={output_dir}/data.yaml  model=yolov8n-seg.pt epochs=1")
    #     # subprocess.run(f"yolo detect train data={output_dir}/data.yaml  model=yolov8n-seg.pt epochs=1", shell=True)
    #     print("开始生成train.cache和val.cache文件")

    train_model = DEDUCTION_MODEL_PATH + '/' + action_name + ".pt"
    # train_model = DEDUCTION_MODEL_PATH + '/' + action_name + "/weights/last.pt"
    print(train_model)
    ...
    # localSavePath = TRAIN_MODEL_SAVE_PATH + "/" + project_name + '/'
    # 本地源文件
    localSavePath = TRAIN_MODEL_SAVE_PATH + "/" + project_name + "/weights/last.pt"
    # 推送后的文件
    remote_path = TRAIN_MODEL_SAVE_PATH + "/" + project_name + ".pt"
    if osName == "linux":
...
#     print("解析csv文件")
#     images_dir = TRAIN_IMAGES_SAVE_PATH
#     # images_dir = TRAIN_IMAGES_SAVE_PATH + '/yyMMdd'
#     csv_path = TRAIN_DATASETS_SAVE_PATH + '/' + request.csv_name
#     output_dir = TRAIN_YOLO_TEMPDATA_PATH + '/' + data_id
#     # csv_to_yolo(images_dir, csv_path, output_dir)
#     csv_to_yolo(images_dir, csv_path, output_dir)
#     # 验证配置文件是否存在
#     if not os.path.exists(TRAIN_YOLO_TEMPDATA_PATH):
...
#         imgsz=request.modelConfig.imgSiz,
#         batch=request.modelConfig.batch,
#         project=request.modelConfig.project,
#         name=request.modelConfig.name,
#         device=request.modelConfig.device
# val = True,
# callbacks = {"on_val_end": logger.log_metrics}  # 验证结束后触发
#     )
